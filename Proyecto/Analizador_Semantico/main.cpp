...
void procesarCondicion(const string& condicion) {
    ...
    if(posAND != string::npos && (posOR == string::npos || posAND < posOR)) {
        ...
        // Procesar subcondiciones recursivamente
        procesarCondicion(cond1);
        procesarCondicion(cond2);
        // Generar cuarteto AND
        string temp1 = "R" + to_string(tempCount - 2); // Penúltima temporal
        string temp2 = "R" + to_string(tempCount - 1); // Última temporal
        ...
        string temp2 = "R" + to_string(tempCount - 1); // Última temporal
        string tempFinal = nuevaTemporal();
        agregarCuarteto("OR", temp1, temp2, tempFinal);
    }
    else {
        // Condición simple
        if(!cond.empty()) {
            ...
            // Procesar la condición simple inmediatamente
            procesarCondicionSimple(cond);
        }
    }
}
// Sentencias SQL
...
void procesarDELETE(const string& linea) {
    ...
    if(wherePos != string::npos) {
        ...
        // Procesar condiciones (pueden tener AND/OR)
        procesarCondicion(condicionCompleta);
        // Usar la última temporal generada
